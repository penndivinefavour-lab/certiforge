// Phase 5.6 Certificate Generation Integration Test (ESM)
...
async function main() {
  ...
  // Test 2: QR Code functionality (using dynamic import)
  console.log('\n[TEST 2] Testing QR code generation...');
  try {
    const qrTest = await import('../../../../packages/qr/dist/index.js');
  ...
  try {
    const certEngine = await import('../../../../packages/certificate-engine/dist/certificate-engine/index.js');
  ...
  try {
    ...
    console.log(e.stdout?.slice(-500) || e.message);
    failed++;
  }
