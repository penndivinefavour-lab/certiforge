...
export async function createSession({ userId, expiresInDays = 7 }: CreateSessionParams) {
...
export async function getUserFromSession(session: Session | null): Promise<SessionUser | null> {
  if (!session) return null;
  return {
    id: session.user.id,
